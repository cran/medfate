 ...
 *  verbose: A flag to print intermediate output
 */
List fb(List x, List soil, double latitude, double slope, double aspect, DataFrame meteo, DataFrame SpParams, 
        DataFrame FuelModelParams, String liveFMCmode = "swb", bool useModelForLive = false, bool verbose=true) {
  if(verbose) Rcout<<"Initializing";
  
  //Prepare swb input
  DataFrame swbInput = forest2swbInput(x, SpParams, soil["dVec"]);
  ...
  for(int i=0;i<numDays;i++) {
    ...
    double delta = 0.4093*sin((2.0*PI*((double)DOY[i])/365.0) - 1.405);
    NumericVector srs = sunRiseSet(latitude*(PI/180.0), aspect*(PI/180.0), slope*(PI/180.0), delta);
