swbgrid<-function(y, SpParams, meteo, dates, 
  ...
  for(i in 1:nCells) {
    ...
    if((!is.na(yid)) && (!is.na(soil))) {             
      xi = forest2swbInput(yid, SpParams, soil$dVec)
  ...
  if(control$verbose) cat(paste(" - number of cells with swbInput == NA: ", sum(is.na(swbInputList)),"\n\n", sep=""))
  ...
  for(day in 1:nDays) {
    if(control$verbose) cat(paste("Day #", day))
